...
When Langfuse credentials are configured, each uncached OpenAI-compatible LLM request is recorded as a Langfuse generation with the request payload, parsed output, token usage when the provider returns it, and metadata such as prompt version, evidence hash, model, and source URL. Semantic and scraper cache hits intentionally skip Langfuse generation logging because no model call occurred.
